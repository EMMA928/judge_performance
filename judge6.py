...
output_directory = r"C:/Users/lixiang/.ssh/n-gram2"
...
def train_and_evaluate_model(df, year_range, vectorizer, selected_features):
    ...
    X_tfidf = pd.DataFrame(X_tfidf.toarray(), columns=vectorizer.get_feature_names_out(), index=df.index)
    ...
    # Ensure alignment
    y = df.loc[X_selected.index, 'leave_one_out_avg_all_years']
    groups = df.loc[X_selected.index, 'opinion_songernames']
